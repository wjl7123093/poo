...
//			}
	...
	private void showLocalMusicList() {
		...
		videoProcessEngine.loadLocalMusic("", new VideoProcessEngine$OnLocalMusicLoadListener() {
...
//			}
	...
	private OnClickListener mThemeRecommendDownloadClickListener = new OnClickListener() {
		...
		public void onClick(final View v) {
			...
			if (theme == null)
				return;
			if (theme.downloadStatus == VideoEffectModel$DownloadStatus.DOWNLOADING || theme.downloadStatus == VideoEffectModel$DownloadStatus.DOWNLOADED) {
				processTheme(theme, v);
				return;
			}
			...
			videoProcessEngine.downloadEffectAsset(VideoEffectFuncModel.THEME_TYPE_RECOMMEND_MV, theme.effectID, new OnAssetDownloadListener() {
				...
				public void onSuccess(final VideoEffectModel videoEffectModel) {
					...
					theme.downloadStatus = VideoEffectModel$DownloadStatus.DOWNLOADED;
					runOnUiThread(new Runnable() {
						public void run() {
	...
	private VideoProcessEngine$OnVideoEncodingListener mOnVideoEncodingListener = new VideoProcessEngine$OnVideoEncodingListener() {
	 ...
	 * 应用音乐主题回调
	 */
	private VideoProcessEngine$OnMusicApplyListener mOnMusicApplyListener = new VideoProcessEngine$OnMusicApplyListener() {
	...
	private VideoProcessEngine$OnVideoPlayListener mOnVideoPlayListener = new VideoProcessEngine$OnVideoPlayListener() {
