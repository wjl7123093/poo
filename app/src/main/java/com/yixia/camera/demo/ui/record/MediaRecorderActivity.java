...
import com.yixia.camera.demo.ui.record.MediaPreviewActivity.CaptureThumbnailsTask;
 ...
 *
 */
public class MediaRecorderActivity extends BaseActivity implements MediaRecorderBase$OnErrorListener, OnClickListener, MediaRecorderBase$OnPreparedListener, com.yixia.weibo.sdk.MediaRecorderBase$OnEncodeListener {

	/** 设置 startActivityForResult() 的返回代码 */
	public final static int RESULT_RECODE = 0x1101;
	...
	private Bitmap mVideoThumbnail = null;	// 视频缩略图
...
//		mRecordDelete.setVisibility(View.VISIBLE);
		mRecordDelete.setVisibility(View.GONE);	/** 屏蔽掉 删除 功能 */
...
//          ContentResolver cr = MediaRecorderActivity.this.getContentResolver();
//			mVideoThumbnail= getVideoThumbnail(MediaRecorderActivity.this, cr, mVideoPath);
			mVideoThumbnail= getVideoThumbnail(mVideoPath, 480, 480,
...
//			startActivity(new Intent(MediaRecorderActivity.this, VideoPlayerActivity.class).putExtra(Constant.RECORD_VIDEO_PATH, mVideoPath).putExtra(Constant.RECORD_VIDEO_CAPTURE, mCoverPath));
			
			Intent data = new Intent();
			data.putExtra("videoPath", mVideoPath);
			data.putExtra("coverPath", mCoverPath);
			data.putExtra("bitmap", mVideoThumbnail);

			// 为了避免红米/魅族手机 调用 setResult() 方法后，不回调 onActivityResult() 方法，
			// 故 将 data 赋值给全局变量 GlobalSet.intentData 来间接获取数据
			GlobalSet.intentData = data;
//			MediaRecorderActivity.this.setResult(RESULT_RECODE, data);
