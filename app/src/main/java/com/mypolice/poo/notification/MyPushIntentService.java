 ...
 * 完全自定义处理类
 *
 * 消息接收时触发，获取消息内容做出自定义处理
 */
public class MyPushIntentService extends UmengMessageService {
    private static final String TAG = MyPushIntentService.class.getName();
    public static int badgeCount = 0;	// 未读消息数目
    ...
    public void onMessage(Context context, Intent intent) {
        try {
            ...
            // 启动 MainActivity.class
            Intent intentAct = new Intent();
            intentAct.setClass(context, MainActivity.class);
            intentAct.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
            ...
            /** 未读数目 +1 */
            ShortcutBadger.applyCount(MyPushIntentService.this, ++badgeCount);
            // 显示通知
