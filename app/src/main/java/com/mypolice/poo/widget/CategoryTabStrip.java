...
/**   
 * @Title: CategoryTabStrip.java 
 * @Package com.mytv.rtmedia.widget
 * @Description: 自定义顶部滑动导航栏（仿今日头条）
 * @author wangjl  
 * @crdate 2017-5-19
 * @update 
 * @version v1.1.2(4)
...
//        right_edge = getResources().getDrawable(R.drawable.ic_category_right_edge);
        right_edge = getResources().getDrawable(R.drawable.ic_category_left_edge);
	}
...
//		// 如果选项卡总宽度 < 父控件宽度，则均分选项卡宽度
//		if (tabWidthTotal <= tabsContainer.getWidth()) {
//			int width = tabsContainer.getWidth() / tabCount;
...
//		}
	...
	public void draw(Canvas canvas) {
        ...
        // 如果选项卡总宽度 < 父控件宽度，则均分选项卡宽度
        int screenWidth = ScreenUtils.getScreenWidth(getContext()) - DensityUtils.dp2px(getContext(), 100);
        // 如果选项卡总数 < 5 或者 选项卡父控件宽度 < 屏幕宽，则取平均值
 		if (tabCount < 5 || tabsContainer.getWidth() <= screenWidth) {
 			int width = screenWidth / tabCount;
 			for (int i = 0; i < tabCount; i++) {
 				ViewGroup tab = (ViewGroup)tabsContainer.getChildAt(i);
            	TextView category_text = (TextView) tab.findViewById(R.id.category_text);
 				tab.setMinimumWidth(width);
 				category_text.setMinimumWidth(width);
 			}
 		}
        ...
        while (i < tabsContainer.getChildCount()) {
            if (i < currentPosition - 1 || i > currentPosition + 1) {
                ...
                if (category_text != null) {
                    ...
                    // 加上此句仅为了该项目的 中藏文切换 功能
                    textDrawable.setTypeface(category_text.getTypeface());
